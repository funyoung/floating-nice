...
import org.programus.android.floatings.model.MemInfo;
 ...
 *
 */
public class MemoryService extends FloatingService {
	private View view;
...
//				long rx = TrafficStats.getMobileRxBytes() + TrafficStats.getTotalRxBytes();
                long rx = MemInfo.getmem_UNUSED(getApplicationContext());
                long tx = MemInfo.getmem_TOLAL();
				if (prevTime >= 0) {
...
//					Log.d("DEBUG", String.format("prx=%d, rx=%d, drx=%f, dt=%d", this.prevRx, rx, drx, dt));
					data.putDouble(UP, tx);
					data.putDouble(DN, rx);
	...
	private void init() {
		...
		this.view.setOnLongClickListener(new OnLongClickListener(){
            ...
            public boolean onLongClick(View v) {
                if (!isMoving()) {
                    SettingsActivity.startActivity(MemoryService.this);
                }
                return false;
            }
		});
...
//		Log.d("DEBUG", String.format("UP: %f / DN: %f", up, down));
	...
	private String getDisplayNumber(double value) {
		...
		return String.format("%6sB", num + UNITS[i]);
	}
